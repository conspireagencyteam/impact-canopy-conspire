...
var _CartDiscount_brand = /*#__PURE__*/new WeakSet();
var CartDiscount = /*#__PURE__*/function (_Component) {
  function CartDiscount() {
    var _this;
    _classCallCheck(this, CartDiscount);
    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }
    _this = _callSuper(this, CartDiscount, [].concat(args));
    _classPrivateMethodInitSpec(_this, _CartDiscount_brand);
    _defineProperty(_this, "requiredRefs", ['cartDiscountError', 'cartDiscountErrorDiscountCode', 'cartDiscountErrorShipping']);
     ...
     */
    _defineProperty(_this, "applyDiscount", /*#__PURE__*/function () {
      var _ref = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(event) {
        var _this$refs, cartDiscountError, cartDiscountErrorDiscountCode, cartDiscountErrorShipping, form, discountCode, discountCodeValue, abortController, existingDiscounts, config, response, data, newHtml, parsedHtml, section, discountCodes, codes, _t;
        return _regenerator().w(function (_context) {
          while (1) switch (_context.p = _context.n) {
            case 0:
              _this$refs = _this.refs, cartDiscountError = _this$refs.cartDiscountError, cartDiscountErrorDiscountCode = _this$refs.cartDiscountErrorDiscountCode, cartDiscountErrorShipping = _this$refs.cartDiscountErrorShipping;
              event.preventDefault();
              ...
              return _context.a(2);
            case 1:
              discountCode = form.querySelector('input[name="discount"]');
              if (!(!(discountCode instanceof HTMLInputElement) || typeof _this.dataset.sectionId !== 'string')) {
                _context.n = 2;
                break;
              }
              return _context.a(2);
            case 2:
              ...
              abortController = _assertClassBrand(_CartDiscount_brand, _this, _createAbortController).call(_this);
              _context.p = 3;
              existingDiscounts = _assertClassBrand(_CartDiscount_brand, _this, _existingDiscounts).call(_this);
            ...
            case 4:
              ...
              config = (0,_theme_utilities__WEBPACK_IMPORTED_MODULE_3__.fetchConfig)('json', {
                body: JSON.stringify({
                  discount: [].concat(_toConsumableArray(existingDiscounts), [discountCodeValue]).join(','),
                  sections: [_this.dataset.sectionId]
                })
              });
              _context.n = 5;
              return fetch(Theme.routes.cart_update_url, _objectSpread(_objectSpread({}, config), {}, {
            ...
            case 6:
              data = _context.v;
              if (!data.discount_codes.find(function (/** @type {{ code: string; applicable: boolean; }} */discount) {
              ...
              discountCode.value = '';
              _assertClassBrand(_CartDiscount_brand, _this, _handleDiscountError).call(_this, 'discount_code');
              return _context.a(2);
            case 7:
              newHtml = data.sections[_this.dataset.sectionId];
              parsedHtml = new DOMParser().parseFromString(newHtml, 'text/html');
              section = parsedHtml.getElementById("shopify-section-".concat(_this.dataset.sectionId));
              discountCodes = (section === null || section === void 0 ? void 0 : section.querySelectorAll('.cart-discount__pill')) || [];
              if (!section) {
                _context.n = 8;
                break;
              }
              codes = Array.from(discountCodes).map(function (element) {
                return element instanceof HTMLLIElement ? element.dataset.discountCode : null;
              }).filter(Boolean); // Before morphing, we need to check if the shipping discount is applicable in the UI
              // we check the liquid logic compared to the cart payload to assess whether we leveraged
              // a valid shipping discount code.
              if (!(codes.length === existingDiscounts.length && codes.every(function (/** @type {string} */code) {
                return existingDiscounts.includes(code);
              }) && data.discount_codes.find(function (/** @type {{ code: string; applicable: boolean; }} */discount) {
                return discount.code === discountCodeValue && discount.applicable === true;
              }))) {
                _context.n = 8;
                break;
              }
              _assertClassBrand(_CartDiscount_brand, _this, _handleDiscountError).call(_this, 'shipping');
              discountCode.value = '';
              return _context.a(2);
            case 8:
              document.dispatchEvent(new _theme_events__WEBPACK_IMPORTED_MODULE_2__.DiscountUpdateEvent(data, _this.id));
              (0,_theme_section_renderer__WEBPACK_IMPORTED_MODULE_1__.morphSection)(_this.dataset.sectionId, newHtml);
              _context.n = 10;
              break;
            case 9:
              _context.p = 9;
            ...
            case 10:
              _context.p = 10;
              _classPrivateFieldSet(_activeFetch, _this, null);
              _theme_performance__WEBPACK_IMPORTED_MODULE_4__.cartPerformance.measureFromEvent('discount-update:user-action', event);
     ...
     */
    _defineProperty(_this, "removeDiscount", /*#__PURE__*/function () {
      var _ref2 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2(event) {
        var pill, discountCode, existingDiscounts, index, abortController, config, response, data, _t2;
        return _regenerator().w(function (_context2) {
          while (1) switch (_context2.p = _context2.n) {
            case 0:
              event.preventDefault();
              event.stopPropagation();
              if (!(event instanceof KeyboardEvent && event.key !== 'Enter' || !(event instanceof MouseEvent) || !(event.target instanceof HTMLElement) || typeof _this.dataset.sectionId !== 'string')) {
                _context2.n = 1;
                break;
              }
              return _context2.a(2);
            case 1:
              pill = event.target.closest('.cart-discount__pill');
              if (pill instanceof HTMLLIElement) {
                _context2.n = 2;
                break;
              }
              return _context2.a(2);
            case 2:
              discountCode = pill.dataset.discountCode;
              if (discountCode) {
                _context2.n = 3;
                break;
              }
            ...
            case 7:
              data = _context2.v;
              document.dispatchEvent(new _theme_events__WEBPACK_IMPORTED_MODULE_2__.DiscountUpdateEvent(data, _this.id));
              (0,_theme_section_renderer__WEBPACK_IMPORTED_MODULE_1__.morphSection)(_this.dataset.sectionId, data.sections[_this.dataset.sectionId]);
              _context2.n = 9;
              break;
            case 8:
              _context2.p = 8;
  ...
  _inherits(CartDiscount, _Component);
  return _createClass(CartDiscount);
 ...
 */
function _handleDiscountError(type) {
  var _this$refs2 = this.refs,
    cartDiscountError = _this$refs2.cartDiscountError,
    cartDiscountErrorDiscountCode = _this$refs2.cartDiscountErrorDiscountCode,
    cartDiscountErrorShipping = _this$refs2.cartDiscountErrorShipping;
  var target = type === 'discount_code' ? cartDiscountErrorDiscountCode : cartDiscountErrorShipping;
 ...
 * @returns {string[]}
 */
function _existingDiscounts() {
  /** @type {string[]} */
  var discountCodes = [];
  var discountPills = this.querySelectorAll('.cart-discount__pill');
  var _iterator = _createForOfIteratorHelper(discountPills),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
