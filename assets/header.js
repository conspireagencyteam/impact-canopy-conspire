 ...
 */
var _menuDrawerHiddenWidth = /*#__PURE__*/new WeakMap();
...
var _animationDelay = /*#__PURE__*/new WeakMap();
var _resizeObserver = /*#__PURE__*/new WeakMap();
var _observeStickyPosition = /*#__PURE__*/new WeakMap();
var _handleOverflowMinimum = /*#__PURE__*/new WeakMap();
var _HeaderComponent_brand = /*#__PURE__*/new WeakSet();
var _handleWindowScroll = /*#__PURE__*/new WeakMap();
var HeaderComponent = /*#__PURE__*/function (_Component) {
  function HeaderComponent() {
    ...
    _this = _callSuper(this, HeaderComponent, [].concat(args));
    /**
     * Updates the visibility of the menu and drawer
     * @param {boolean} hideMenu - Whether to hide the menu and show the drawer
     */
    _classPrivateMethodInitSpec(_this, _HeaderComponent_brand);
    _defineProperty(_this, "requiredRefs", ['headerDrawerContainer', 'headerMenu', 'headerRowTop']);
    /**
     * Width of window when header drawer was hidden
     * @type {number | null}
     */
    _classPrivateFieldInitSpec(_this, _menuDrawerHiddenWidth, null);
    /**
     * An intersection observer for monitoring sticky header position
     * @type {IntersectionObserver | null}
     */
    _classPrivateFieldInitSpec(_this, _intersectionObserver, null);
    /**
     * Whether the header has been scrolled offscreen, when sticky behavior is 'scroll-up'
     * @type {boolean}
     */
    _classPrivateFieldInitSpec(_this, _offscreen, false);
    /**
     * The last recorded scrollTop of the document, when sticky behavior is 'scroll-up
     * @type {number}
     */
    _classPrivateFieldInitSpec(_this, _lastScrollTop, 0);
    /**
     * A timeout to allow for hiding animation, when sticky behavior is 'scroll-up'
     * @type {number | null}
     */
    _classPrivateFieldInitSpec(_this, _timeout, null);
    /**
     * The duration to wait for hiding animation, when sticky behavior is 'scroll-up'
     * @constant {number}
     */
    _classPrivateFieldInitSpec(_this, _animationDelay, 150);
    /**
     * Keeps the global `--header-height` custom property up to date,
     * which other theme components can then consume
     */
    _classPrivateFieldInitSpec(_this, _resizeObserver, new ResizeObserver(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 1),
        entry = _ref2[0];
      if (!entry) return;
      var _entry$target$getBoun = entry.target.getBoundingClientRect(),
        height = _entry$target$getBoun.height;
      document.body.style.setProperty('--header-height', "".concat(height, "px"));

      // Check if the menu drawer should be hidden in favor of the header menu
      if (_classPrivateFieldGet(_menuDrawerHiddenWidth, _this) && window.innerWidth > _classPrivateFieldGet(_menuDrawerHiddenWidth, _this)) {
        _assertClassBrand(_HeaderComponent_brand, _this, _updateMenuVisibility).call(_this, false);
      }
    }));
    /**
     ...
     */
    _classPrivateFieldInitSpec(_this, _observeStickyPosition, function () {
      var alwaysSticky = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
      if (_classPrivateFieldGet(_intersectionObserver, _this)) return;
      var config = {
        threshold: alwaysSticky ? 1 : 0
      };
      _classPrivateFieldSet(_intersectionObserver, _this, new IntersectionObserver(function (_ref3) {
        var _ref4 = _slicedToArray(_ref3, 1),
          entry = _ref4[0];
        if (!entry) return;
        var isIntersecting = entry.isIntersecting;
        if (alwaysSticky) {
          _this.dataset.stickyState = isIntersecting ? 'inactive' : 'active';
          (0,_theme_utilities__WEBPACK_IMPORTED_MODULE_2__.changeMetaThemeColor)(_this.refs.headerRowTop);
        } else {
          _classPrivateFieldSet(_offscreen, _this, !isIntersecting || _this.dataset.stickyState === 'active');
        }
      }, config));
      _classPrivateFieldGet(_intersectionObserver, _this).observe(_this);
    });
    /**
     * Handles the overflow minimum event from the header menu
     * @param {OverflowMinimumEvent} event
     */
    _classPrivateFieldInitSpec(_this, _handleOverflowMinimum, function (event) {
      _assertClassBrand(_HeaderComponent_brand, _this, _updateMenuVisibility).call(_this, event.detail.minimumReached);
    });
    _classPrivateFieldInitSpec(_this, _handleWindowScroll, function () {
      var _document$scrollingEl, _document$scrollingEl2;
      var stickyMode = _this.getAttribute('sticky');
      if (!_classPrivateFieldGet(_offscreen, _this) && stickyMode !== 'always') return;
      var scrollTop = (_document$scrollingEl = (_document$scrollingEl2 = document.scrollingElement) === null || _document$scrollingEl2 === void 0 ? void 0 : _document$scrollingEl2.scrollTop) !== null && _document$scrollingEl !== void 0 ? _document$scrollingEl : 0;
      var isScrollingUp = scrollTop < _classPrivateFieldGet(_lastScrollTop, _this);
      if (_classPrivateFieldGet(_timeout, _this)) {
        clearTimeout(_classPrivateFieldGet(_timeout, _this));
        _classPrivateFieldSet(_timeout, _this, null);
      }
      if (stickyMode === 'always') {
        var isAtTop = _this.getBoundingClientRect().top >= 0;
        if (isAtTop) {
          _this.dataset.scrollDirection = 'none';
        } else if (isScrollingUp) {
          _this.dataset.scrollDirection = 'up';
        } else {
          _this.dataset.scrollDirection = 'down';
        }
        _classPrivateFieldSet(_lastScrollTop, _this, scrollTop);
        return;
      }
      if (isScrollingUp) {
        _this.removeAttribute('data-animating');
        if (_this.getBoundingClientRect().top >= 0) {
          // reset sticky state when header is scrolled up to natural position
          _classPrivateFieldSet(_offscreen, _this, false);
          _this.dataset.stickyState = 'inactive';
          _this.dataset.scrollDirection = 'none';
        } else {
          // show sticky header when scrolling up
          _this.dataset.stickyState = 'active';
          _this.dataset.scrollDirection = 'up';
        }
      } else if (_this.dataset.stickyState === 'active') {
        _this.dataset.scrollDirection = 'none';
        // delay transitioning to idle hidden state for hiding animation
        _this.setAttribute('data-animating', '');
        _classPrivateFieldSet(_timeout, _this, setTimeout(function () {
          _this.dataset.stickyState = 'idle';
          _this.removeAttribute('data-animating');
        }, _classPrivateFieldGet(_animationDelay, _this)));
      } else {
        _this.dataset.scrollDirection = 'none';
        _this.dataset.stickyState = 'idle';
      }
  ...
  return _createClass(HeaderComponent, [{
    key: "connectedCallback",
    value: function connectedCallback() {
      _superPropGet(HeaderComponent, "connectedCallback", this, 3)([]);
      _classPrivateFieldGet(_resizeObserver, this).observe(this);
      this.addEventListener('overflowMinimum', _classPrivateFieldGet(_handleOverflowMinimum, this));
      var stickyMode = this.getAttribute('sticky');
      if (stickyMode) {
        _classPrivateFieldGet(_observeStickyPosition, this).call(this, stickyMode === 'always');
        if (stickyMode === 'scroll-up' || stickyMode === 'always') {
          document.addEventListener('scroll', _classPrivateFieldGet(_handleWindowScroll, this));
        }
      }
    }
  }, {
    ...
    value: function disconnectedCallback() {
      ...
      _superPropGet(HeaderComponent, "disconnectedCallback", this, 3)([]);
      _classPrivateFieldGet(_resizeObserver, this).disconnect();
      ...
      this.removeEventListener('overflowMinimum', _classPrivateFieldGet(_handleOverflowMinimum, this));
      document.removeEventListener('scroll', _classPrivateFieldGet(_handleWindowScroll, this));
      document.body.style.setProperty('--header-height', '0px');
    }
  }]);
}(_theme_component__WEBPACK_IMPORTED_MODULE_1__.Component);
function _updateMenuVisibility(hideMenu) {
  if (hideMenu) {
    this.refs.headerDrawerContainer.classList.remove('desktop:hidden');
    _classPrivateFieldSet(_menuDrawerHiddenWidth, this, window.innerWidth);
    this.refs.headerMenu.classList.add('hidden');
  } else {
    this.refs.headerDrawerContainer.classList.add('desktop:hidden');
    _classPrivateFieldSet(_menuDrawerHiddenWidth, this, null);
...
(0,_theme_utilities__WEBPACK_IMPORTED_MODULE_2__.onDocumentLoaded)(function () {
  var header = document.querySelector('#header-component');
  var headerGroup = document.querySelector('#header-group');
  ...
  if (headerGroup) {
    var resizeObserver = new ResizeObserver(function () {
      return (0,_theme_critical__WEBPACK_IMPORTED_MODULE_0__.calculateHeaderGroupHeight)(header, headerGroup);
    });
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          if (mutation.type === 'childList') {
            ...
            var _children = headerGroup.children;
            for (var _i = 0; _i < _children.length; _i++) {
              var _element = _children[_i];
              if (_element === header || !(_element instanceof HTMLElement)) continue;
