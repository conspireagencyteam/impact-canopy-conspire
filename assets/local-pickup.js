...
(() => {
/*!*************************************!*\
  !*** ./src/scripts/local-pickup.js ***!
  \*************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _theme_component__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @theme/component */ "./src/scripts/component.js");
/* harmony import */ var _theme_morph__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @theme/morph */ "./src/scripts/morph.js");
...
var _LocalPickup_brand = /*#__PURE__*/new WeakSet();
var _fetchAvailability = /*#__PURE__*/new WeakMap();
var LocalPickup = /*#__PURE__*/function (_Component) {
  function LocalPickup() {
    var _this;
    _classCallCheck(this, LocalPickup);
    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }
    _this = _callSuper(this, LocalPickup, [].concat(args));
    _classPrivateMethodInitSpec(_this, _LocalPickup_brand);
    /** @type {AbortController | undefined} */
    _classPrivateFieldInitSpec(_this, _activeFetch, void 0);
    /**
     * Fetches the availability of a variant.
     * @param {string} variantId - The ID of the variant to fetch availability for.
     */
    _classPrivateFieldInitSpec(_this, _fetchAvailability, function (variantId) {
      if (!variantId) return;
      var abortController = _assertClassBrand(_LocalPickup_brand, _this, _createAbortController).call(_this);
      var url = _this.dataset.productUrl;
      fetch("".concat(url, "?variant=").concat(variantId, "&section_id=").concat(_this.dataset.sectionId), {
        ...
        var wrapper = html.querySelector("local-pickup[data-variant-id=\"".concat(variantId, "\"]"));
        if (wrapper) {
          _this.removeAttribute('hidden');
          (0,_theme_morph__WEBPACK_IMPORTED_MODULE_1__.morph)(_this, wrapper);
        } else _this.setAttribute('hidden', '');
      }).catch(function (_e) {
        if (abortController.signal.aborted) return;
  ...
  _inherits(LocalPickup, _Component);
  return _createClass(LocalPickup, [{
    key: "connectedCallback",
    value: function connectedCallback() {
      var _this2 = this;
      _superPropGet(LocalPickup, "connectedCallback", this, 3)([]);
      var closestSection = this.closest(".shopify-section, dialog");

      /** @type {(event: VariantUpdateEvent) => void} */
      var variantUpdated = function variantUpdated(event) {
        if (event.detail.data.newProduct) {
          _this2.dataset.productUrl = event.detail.data.newProduct.url;
        }
        var variantId = event.detail.resource ? event.detail.resource.id : null;
        var variantAvailable = event.detail.resource ? event.detail.resource.available : null;
        if (variantId !== _this2.dataset.variantId) {
          if (variantId && variantAvailable) {
            _this2.removeAttribute('hidden');
            _this2.dataset.variantId = variantId;
            _classPrivateFieldGet(_fetchAvailability, _this2).call(_this2, variantId);
          } else {
            _this2.setAttribute('hidden', '');
          }
        }
      };
      closestSection === null || closestSection === void 0 || closestSection.addEventListener(_theme_events__WEBPACK_IMPORTED_MODULE_2__.ThemeEvents.variantUpdate, variantUpdated);
      this.disconnectedCallback = function () {
        closestSection === null || closestSection === void 0 || closestSection.removeEventListener(_theme_events__WEBPACK_IMPORTED_MODULE_2__.ThemeEvents.variantUpdate, variantUpdated);
      };
    }
  }]);
}(_theme_component__WEBPACK_IMPORTED_MODULE_0__.Component);
function _createAbortController() {
  if (_classPrivateFieldGet(_activeFetch, this)) _classPrivateFieldGet(_activeFetch, this).abort();
  _classPrivateFieldSet(_activeFetch, this, new AbortController());
  return _classPrivateFieldGet(_activeFetch, this);
}
