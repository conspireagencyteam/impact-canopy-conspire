 ...
 */
var _JumboText_brand = /*#__PURE__*/new WeakSet();
var _calculateOptimalFontSize = /*#__PURE__*/new WeakMap();
var _handleResize = /*#__PURE__*/new WeakMap();
var _resizeObserver = /*#__PURE__*/new WeakMap();
var JumboText = /*#__PURE__*/function (_HTMLElement) {
  function JumboText() {
    var _this;
    _classCallCheck(this, JumboText);
    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }
    _this = _callSuper(this, JumboText, [].concat(args));
    /**
     * Sets the intersection observer to calculate the optimal font size when the text is in view
     */
    _classPrivateMethodInitSpec(_this, _JumboText_brand);
    /**
     * Calculates the optimal font size to make the text fit the container width
     */
    _classPrivateFieldInitSpec(_this, _calculateOptimalFontSize, function () {
      var _this$textContent;
      // Check for empty text
      if (!((_this$textContent = _this.textContent) !== null && _this$textContent !== void 0 && _this$textContent.trim())) {
        return;
      }

      // Hide text during calculation
      _this.classList.remove('ready');
      if (_this.offsetWidth <= 0) return;

      // Disconnect the resize observer
      _classPrivateFieldGet(_resizeObserver, _this).disconnect();

      // Start with a minimal font size
      _this.style.fontSize = '1px';

      // Find the optimal font size
      var fontSize = findOptimalFontSize(_this, _this.offsetWidth);

      // Apply the final size
      _this.style.fontSize = "".concat(fontSize, "px");

      // Reconnect the resize observer
      _classPrivateFieldGet(_resizeObserver, _this).observe(_this);

      // Show the text
      _this.classList.add('ready');
    });
    _classPrivateFieldInitSpec(_this, _handleResize, function () {
      _classPrivateFieldGet(_calculateOptimalFontSize, _this).call(_this);
      ...
      var distanceFromBottom = document.documentElement.offsetHeight - bottom;
      _this.dataset.capText = (distanceFromBottom <= 100).toString();
    });
    _classPrivateFieldInitSpec(_this, _resizeObserver, new _theme_critical__WEBPACK_IMPORTED_MODULE_0__.ResizeNotifier(_classPrivateFieldGet(_handleResize, _this)));
    return _this;
  }
  _inherits(JumboText, _HTMLElement);
  return _createClass(JumboText, [{
    key: "connectedCallback",
    value: function connectedCallback() {
      // Initial calculation
      requestAnimationFrame(_classPrivateFieldGet(_handleResize, this));
      if (this.dataset.textEffect && this.dataset.textEffect !== 'none' && !(0,_theme_utilities__WEBPACK_IMPORTED_MODULE_1__.prefersReducedMotion)()) {
        _assertClassBrand(_JumboText_brand, this, _setIntersectionObserver).call(this);
      }
    }
  }, {
    key: "disconnectedCallback",
 ...
 */
function _setIntersectionObserver() {
  var _this2 = this;
  // The threshold could be different based on the repetition of the animation.
  this.intersectionObserver = new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      if (entry.isIntersecting) {
        _this2.classList.add('jumbo-text-visible');
        if (_this2.dataset.animationRepeat === 'false') {
          _this2.intersectionObserver.unobserve(entry.target);
        }
      } else {
        _this2.classList.remove('jumbo-text-visible');
      }
    });
  }, {
    threshold: 0.3
...
function findOptimalFontSize(element, containerWidth) {
  ...
  var textLength = ((_element$textContent = element.textContent) === null || _element$textContent === void 0 ? void 0 : _element$textContent.length) || 0;
  var fontSize = Math.min(maxSize, Math.sqrt(containerWidth) * (15 / Math.sqrt(Math.max(1, textLength))));
